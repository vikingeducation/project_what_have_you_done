...
router.use('/:name', function(req, res, next) {
  ...
  var options = {
    url: 'https://congress.api.sunlightfoundation.com/' +
      'votes?fields=voter_ids,question,bill&voter_ids.' +
      representative.id +
      '__exists=true&bill_id__exists=true&vote_type=passage'
  };
    ...
    if (!error && response.statusCode == 200) {
      ...
      for (var i = 0; i < info.results.length; i++) {
        if ('bill' in info.results[i]) {
          votes.push(
            new Vote(
              info.results[i].bill.bill_id,
              info.results[i].bill.urls.govtrack,
              info.results[i].bill.official_title,
              info.results[i].voter_ids[representative.id] === 'Yea'
                ? true
                : false
            )
          );
