...
const sunlight = {
  ...
  getVotes: function(legislator, chamber, callback) {
    ...
    const url = baseUrl + `votes?chamber=${chamber}&vote_type=passage&order=voted_at&fields=${fields}`;
    const options = {
      uri: url,
      json: true
    }
    rp(options)
      .then(function(response) {
        ...
        response.results.forEach(function(vote) {
          // only grab bills our pol voted on
